...
/* LIN_RqFrame - Source code (requires t5vm.h include) */
/* Function */
...
#define _P_LENGTH   (*((T5_PTBYTE)(T5GET_DBDATA8(pBase))+pArgs[2]))
#define _P_RET   (*((T5_PTSHORT)(T5GET_DBDATA16(pBase))+pArgs[3]))

/* handler */

T5_DWORD LIN_RQFRAME (
    T5_WORD wCommand,
    T5PTR_DB pBase,
    T5_PTR pClass,
    T5_PTR pInst,
    T5_PTWORD pArgs)
{
   ...
   case T5FBCMD_ACTIVATE :
        _P_RET=DstWMonitor_eApplication_Function.s32MLin_eReadRequest_Exe( _P_FRAMEPID, _P_LENGTH, _P_LINVERS);
      return 0L;
