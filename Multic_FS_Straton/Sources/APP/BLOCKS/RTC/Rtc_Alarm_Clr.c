...
/* RTC_Alarm_CLR - Source code (requires t5vm.h include) */
/* Function block */
...
} _str_FB_RTC_ALARM_CLR;

/* handler */

T5_DWORD RTC_ALARM_CLR (
    T5_WORD wCommand,
    T5PTR_DB pBase,
    T5_PTR pClass,
    T5_PTR pInst,
    T5_PTWORD pArgs)
{
   ...
   case T5FBCMD_ACTIVATE :
      ...
      if (TRUE==_P_CMD)
      {
					...
					{
		      		s32ReturnValue=DstWMonitor_eApplication_Function.ps32ARtc_eAlarm_Set(NULL);
   ...
   case T5FBCMD_SIZEOFINSTANCE :
      /* dont change that */
      return (T5_DWORD)sizeof(_str_FB_RTC_ALARM_CLR);
